...
import {
  ...
  getFreeRunsUsed,
  isSubscriptionActive as isSubscriptionActiveHelper,
  consumeFreeRun as consumeLocalFreeRun,
...
export function useSubscription() {
  ...
  const [subscription, setSubscription] = useState<SubscriptionRecord | null>(() => getActiveSubscription());
  const [history, setHistory] = useState<SubscriptionRecord[]>(() => getSubscriptionHistory());
  const [isActive, setIsActive] = useState<boolean>(() => isSubscriptionActiveHelper() || isAdminUnlocked());
  const [isAdmin, setIsAdmin] = useState<boolean>(() => isAdminUnlocked());
  const [freeTrialUsed, setFreeTrialUsed] = useState<boolean>(() => getFreeRunsUsed() >= 1);
  const [hasFreeRun, setHasFreeRun] = useState<boolean>(() => getFreeRunsUsed() < 1);
  ...
  const loadWalletData = useCallback(async (walletAddr: string) => {
    ...
    if (!walletAddr) {
      ...
      const localUsed = getFreeRunsUsed() >= 1;
      setFreeTrialUsed(localUsed);
      setHasFreeRun(!localUsed);
      const subActive = isSubscriptionActiveHelper();
      setIsActive(subActive || admin);
    ...
    try {
      ...
      if (record) {
        ...
        if (record.free_trial_used) {
          try {
            localStorage.setItem('cryptopay_free_trial_runs_count', '1');
          } catch {
            // ignore
          }
        }
        ...
        // Brand new wallet not yet recorded: check local storage state
        const localUsed = getFreeRunsUsed() >= 1;
        setFreeTrialUsed(localUsed);
        setHasFreeRun(!localUsed);
        setIsActive(admin);
  ...
  const handleConsumeFreeRun = useCallback(async (): Promise<{
    ...
    // 1. Immediately consume local run so UI updates instantly
    consumeLocalFreeRun();
  ...
  if (isAdmin || isActive || hasFreeRun) {
    canRun = true;
  } else {
    canRun = false;
    runReason = 'Free 1st run used. Please upgrade your subscription to continue running invoices.';
  }
